...
int SDU_Processor::get_sdu(uint8_t vcid, M_SDU &sdu){
	...
	DEBUG("%s","Have a pdu of length: %d\n", sdu.length+1 );
	...
	while( pkgdata < sdu.length+1 && pdu != buffer.end()  ){
		DEBUG("%s","Cur First Pointer: %d\n", pdu->first_pointer );
		if ( pdu->first_pointer == PDU_NO_HEADER ){
			//middle PDU for SDU
			// we just copy the whole thing
			DEBUG("%s","Adding full packet zone to M_SDU (884 bytes)\n", __FILE__, __LINE__ );
			...
			if( pkgdata == 0){
				...
				DEBUG("%s", "Adding %d bytes from packet zone to M_SDU (first pdu)\n", num_bytes_to_copy );
			...
			//  TODO: may also need to check this using sdu;
				DEBUG("%s","Adding %d bytes from packet zone to M_SDU (middle/end pdu)\n", pdu->first_pointer );
	...
	DEBUG( "%s","Have pulled %d bytes for packet of size %d bytes\n", pkgdata, sdu.length+1 );	
